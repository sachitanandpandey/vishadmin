...
export default {
  ...
  setup () {
    ...
    // data.previewimg = data.defaultProfile.name
    ...
    const submit = async () => {
      ...
      const storageReflist = ref(storage, 'config_images/' + data.defaultProfile.name)
      ...
      const uploaddefaultprofile = await uploadBytesResumable(storageReflist, data.defaultProfile, metadata)
      ...
      setDoc(doc(colRef, 'vishconfig'), {
        // title: data.title,
        // desc: data.desc,
        // status: data.status,
        defaultprofile: defaultPosterUrl
      ...
      const querySnapshotProjects = await getDocs(collection(db, 'config'))
    ...
    const pupdate = async (item) => {
      data.defaultProfile = item.defaultProfile
    //   data.desc = item.desc
